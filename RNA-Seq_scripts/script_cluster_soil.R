...
deganno <- read_csv('eachGroup_vs_Mock_k_soil.csv',
...
sampleN <- c('Mock', 'SynCom33', 'SynCom35')
meanCount <- rawCount %>%
  apply(1, meanFlg22) %>%
  t
...
ggsave('kmeans_sse_soil.pdf')
ggsave('kmeans_sse_soil.jpg')
...
ggsave('kmeans_AIC_soil.pdf')
ggsave('kmeans_AIC_soil.jpg')
...
ggsave(paste0(prefix, '_soil.pdf'))
ggsave(paste0(prefix, '_soil.jpg'))
...
ggsave(paste0(prefix, '_genes_soil.pdf'), width = 10, dpi = 320)
ggsave(paste0(prefix, '_genes_soil.jpg'), width = 10, dpi = 320)
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

##~~~~~~~~~~~~~~~~~~~~~~~~cluster cor phenotype~~~~~~~~~~~~~~~~~
traits <- data.frame(SynCom33 = c(0, 1, 0),
                     SynCom35 = c(0, 0, 1),
                     LiveBacteria = c(0, 1, 1))
...
ggplot(traitCorPlot, aes(x = x, y = y, fill = correlation)) +
  ...
  ylab('Cluster') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(paste0(prefix, '_trait_soil.jpg'), height = 8)
ggsave(paste0(prefix, '_trait_soil.pdf'), height = 8)
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
degresC <- deganno %>%
  select(ID, SynCom33_vs_Mock_pvalue : SynCom33_vs_SynCom35_log2FoldChange)
...
inner_join(deganno, heatPlot) %>%
  ...
  write_csv(paste0(prefix, '_soil.csv'))
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#################################################################
